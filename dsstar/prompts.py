...
def coder_prompt(
    ...
    return (
        ...
        + "- Read repository inputs using os.environ['DSSTAR_REPO_ROOT'] (for example REPO_ROOT / 'input').\n"
        + "- Write ALL outputs/intermediate files under os.environ['DSSTAR_RUN_DIR'].\n"
        + "- Never write outside DSSTAR_RUN_DIR. Relative writes are only allowed when they stay in DSSTAR_RUN_DIR.\n"
        + "- Do not modify any file under DSSTAR_REPO_ROOT.\n"
        + "- If a repo file update is needed (for example knowledge/master.py), write a proposal under DSSTAR_PROPOSE_DIR using repo-relative paths.\n"
        + "- Always include DSSTAR_PROPOSE_DIR/README.md with a short rationale for proposed repo changes.\n"
        + "Copyable setup snippet:\n"
        + "from pathlib import Path\n"
        + "import os\n"
        + "REPO_ROOT = Path(os.environ['DSSTAR_REPO_ROOT']).resolve()\n"
        + "RUN_DIR = Path(os.environ['DSSTAR_RUN_DIR']).resolve()\n"
        + "PROPOSE_DIR = Path(os.environ['DSSTAR_PROPOSE_DIR']).resolve()\n"
        + "INPUT_DIR = REPO_ROOT / 'input'\n"
        + "OUT_DIR = RUN_DIR / 'artifacts'\n"
        + "OUT_DIR.mkdir(parents=True, exist_ok=True)\n"
        + "target_rel = Path('knowledge') / 'master.py'\n"
        + "proposed_path = PROPOSE_DIR / target_rel\n"
        + "proposed_path.parent.mkdir(parents=True, exist_ok=True)\n"
        + "# write proposed_path (NOT REPO_ROOT / 'knowledge' / 'master.py')\n"
        + f"Question: {question}\n"
